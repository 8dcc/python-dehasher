...
try:
    ...
    print(" [i] Required packages: sys, os, requests, bs4, colorama, time, argparse")
    print()
    exit(1)
#--------------------------------------------------------------------------------------------------
...
parser.add_argument('-m', '--method', choices=['single', 'hashlist', 'email'], default="single")
parser.add_argument('-t', '--target', required=True)
# python sample.py -m single -t "asinglehash"

args = parser.parse_args()
# print(args.target)

# Check for errors
if args.single is not None and ".txt" in args.target:
    print(" %s%s[!] Error. Type -l to use a hashlist.%s" % (Style.BRIGHT, Fore.RED, Fore.RESET))
if (args.hashlist is not None or args.email is not None) and ".txt" not in args.target:
